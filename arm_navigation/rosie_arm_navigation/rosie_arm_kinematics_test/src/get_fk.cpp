...
int main(int argc, char **argv){
  ...
  ros::service::waitForService("rosie_right_arm_kinematics/get_fk_solver_info");
  ros::service::waitForService("rosie_right_arm_kinematics/get_fk");

  ros::ServiceClient query_client = 
    rh.serviceClient<kinematics_msgs::GetKinematicSolverInfo>
    ("rosie_right_arm_kinematics/get_fk_solver_info");
  ros::ServiceClient fk_client = rh.serviceClient
    <kinematics_msgs::GetPositionFK>("rosie_right_arm_kinematics/get_fk");
  ...
  fk_request.header.frame_id = "calib_right_arm_base_link";
  fk_request.header.frame_id = "base_link";
  fk_request.fk_link_names.resize(2);
  fk_request.fk_link_names[0] = "right_arm_hand_link";
  fk_request.fk_link_names[1] = "right_arm_2_link";
  ...
  for(unsigned int i=0; 
      i< response.kinematic_solver_info.joint_names.size(); i++)
  {
    fk_request.robot_state.joint_state.position[i] = 0.1;
  }
