/*  $Id: owl.pl,v 1.45 2007/03/23 10:43:59 jan Exp $
...
%% BK: Changed all occurrences of rdfs_individual_of to rdfs_instance_of to incorporate computables...
%%
...
owl_restriction_on(Class, Restriction) :-
  ...
  (   rdfs_instance_of(Super, owl:'Restriction'),
...
cardinality_on_subject(Subject, Predicate, cardinality(Min, Max)) :-
  ...
  rdfs_instance_of(RestrictionID, owl:'Restriction'),
...
owl_satisfies_restriction(Resource, Restriction) :-
  setof(MyProperty, rdf_has(Restriction, owl:onProperty, MyProperty), Properties),
  member(Property, Properties),
  (   rdf_has(Restriction, owl:hasValue, Value)
  ->  rdf_triple(Property, Resource, Value)
...
owl_satisfies_cardinality(Resource, Property, Restriction) :-
  ...
  length(Vs, Count),
  Count =< Max.
owl_satisfies_cardinality(_Resource, _, _) :-
  true. % BK: TODO: rdf_subject doesn't work for computables here
%  rdf_subject(Resource).
...
owl_individual_of(Resource, Class) :-
  rdfs_instance_of(Class, owl:'Class'),
  (   rdf_has(Class, owl:equivalentClass, EQ)
  ->  owl_individual_of(Resource, EQ) % BK: Inifinite recursion???
  ;   rdfs_instance_of(Class, owl:'Restriction')
...
owl_individual_of_description(Resource, Description) :-
  ...
  ;   fail			% not an OWL description, BK: changed from true to fail!
  ).
...
owl_has(S, P, O) :-
  (   var(P)
  ->  rdfs_instance_of(P, rdf:'Property')
  ;   true
  ),
...
owl_has_transitive(S, P, O) :-
  rdfs_instance_of(P, owl:'TransitiveProperty'), !,
...
owl_has_direct(S, P, O) :-
  rdfs_instance_of(P, owl:'SymmetricProperty'),
  rdf(O, P, S).
owl_has_direct(S, P, O) :-
