...
:- dynamic jython_cached_interpreter/1.
:- dynamic user:jythonPath/1.
:- multifile user:jythonPath/1.
jythonInterpreter(Interpreter) :- jython_cached_interpreter(Interpreter), !.
jythonInterpreter(Interpreter) :-
   findall(J, user:jythonPath(J), Js),
   (length(Js, 0) ->  
      Path='.';
      concat_atom(Js, ':', Path)),
   jython_init(Path, Interpreter),
   assert(jython_cached_interpreter(Interpreter)).
...
rdfs_computable_sql_class(Class, ComputableClass) :-
  ...
  rdfs_individual_of(ComputableClass,computable:'SqlClass').
...
rdfs_computable_sql_instance_of(Instance, Class) :-
  nonvar(Instance),!,
  atom(Instance), % Is this needed for split_url to not throw an exception?
...
rdfs_computable_prolog_class(Class, ComputableClass) :-
  ...
  % Destroy all choicepoints for rdf_has
...
%
% Unify ComputableProperty with the "computable property"
% ot Property.
%
%%
rdfs_computable_property(Property, ComputableProperty) :-
  rdfs_computable_sql_property(Property, ComputableProperty)
  ; rdfs_computable_jython_property(Property, ComputableProperty)
...
rdfs_computable_triple_1(Property, Frame, Value) :-
  ...
  ; catch(catch(rdfs_computable_jython_triple(Property, Frame, Value), error(instantiation_error, _), fail),
      error(jave_exception(JE), ExceptionRest), 
...
%%
% Get the parameterNames of a computable jython property.
%%
rdfs_computable_jython_parameter_names(ComputableProperty, ParameterNames) :-
  nonvar(ComputableProperty),
  findall(Parameter, rdf_has(ComputableProperty, computable:'hasParameter', Parameter), ParameterNames).

%%
% Compute the value of a named parameter from its :hasValue slot and
% return a key value pair where the key comes from the :hasName slot.
%%
rdfs_computable_jython_parameter(ParameterName, Frame, ParameterMap) :-
  rdf_has(ParameterName, computable:hasName, RealName),
  rdf_has(ParameterName, computable:hasValue, Value),
  (RealName = literal(type(_,Name))
  -> true
  ; concat_atom([RealName, " is not a literal parameter name!"], Message),
      throw(error(Message, _))),
  (Value = literal(type(_,TempValue))
  -> ParameterMap = [Name, TempValue]
  ; (rdfs_individual_of(Value, owl:'FunctionalProperty')
    -> rdf_triple(Value, Frame, ParameterValue)
    ; setof(PV,rdf_triple(Value, Frame, PV), ParameterValue)
    ),
    ParameterMap = [RealName, ParameterValue]).

%%
% Compute all parameters in a list of named Parameters via rdfs_computable_jython_parameter.
%%
rdfs_computable_jython_parameters([], _, []).
rdfs_computable_jython_parameters([ParameterName| Rest], Frame, [ParameterValue| RestValues]) :-
  rdfs_computable_jython_parameter(ParameterName, Frame, ParameterValue),
  rdfs_computable_jython_parameters(Rest, Frame, RestValues).

%%
% Initialize the parameters in the interpreter for the jython call.
%%
rdfs_computable_jython_init_parameters(Interpreter, Parameters) :-
  var(Interpreter),!,
  jythonInterpreter(Interpreter),
  rdfs_computable_jython_init_parameters(Interpreter, Parameters).
rdfs_computable_jython_init_parameters(_, []).
rdfs_computable_jython_init_parameters(Interpreter, [[Key, Value]| Parameters]) :-
  nonvar(Interpreter),
  rdfs_computable_literal_value(Key, MyKey),
  (nonvar(Value)
  -> (Value=literal(type(_,MyValue))
     -> true
     ; MyValue = Value)),
  jython_variable(Interpreter, MyKey, MyValue),
  rdfs_computable_jython_init_parameters(Interpreter, Parameters).

%%
% ComputableProperty is a JythonProperty with target Property?
%%
rdfs_computable_jython_property(Property, ComputableProperty) :-
  % Destroy all choicepoints for rdf_has
  findall(CP, rdf_has(CP,computable:'target',Property), ComputableProperties),
  member(ComputableProperty, ComputableProperties),
  once(rdfs_individual_of(ComputableProperty,computable:'JythonProperty')).

%%
% rdfs_computable_jython_triple(+Property, +Frame, ?Value)
%
% Value is the value of a jython computable property Property with frame Frame.
%%
rdfs_computable_jython_triple(Property, _, _) :-
  var(Property),!,throw(error(instantiation_error, _)).
rdfs_computable_jython_triple(_, Frame, _) :-
  var(Frame),!,throw(error(instantiation_error, _)).
rdfs_computable_jython_triple(Property, Frame, Value) :-
  nonvar(Frame),
  rdfs_computable_jython_property(Property, ComputableProperty),
  rdfs_computable_jython_parameter_names(ComputableProperty, ParameterNames),
  rdfs_computable_jython_parameters(ParameterNames, Frame, Parameters),
  rdf_has(ComputableProperty, computable:command, literal(type(_, Command))),
  rdf_has(ComputableProperty, computable:nameOfReturn, literal(type(_, ReturnName))),
  rdfs_computable_jython_init_parameters(Interpreter, Parameters),
  jython_variable(Interpreter, frame, Frame),
  
  % Moritz:  
  %   jython-variable value binden, wenn Value nonvar ist? -> wuerde test-abfragen 
  %   erlauben und nicht nur auslese-abfragen
  (nonvar(Value) 
    -> jython_variable(Interpreter, value, Value)
    ; true),
  
  (rdf_has(ComputableProperty, computable:isType, literal(type(_, script)))
  -> jython_exec_script(Interpreter, Command, [ReturnName], [JythonValue])
  ; rdf_has(ComputableProperty, computable:isType, literal(type(_, command)))
  -> jython_call(Interpreter, Command, [ReturnName], [JythonValue])
  ; fail),
  jython:jython_to_prolog(JythonValue, PrologValue),
  ( PrologValue=[_|_] 
  -> member(Temp, PrologValue),
...
%%
% rdfs_computable_prolog_triple(Property, _, _) :-
%   var(Property),!,throw(error(instantiation_error, _)).
% rdfs_computable_prolog_triple(_, Frame, _) :-
%   var(Frame),!,throw(error(instantiation_error, _)).
...
rdfs_computable_prolog_triple(Property, Frame, Value) :-
  ...
  % Lars:
  % wenn Frame und Value bereits Bindings haben, Command direkt mit den Bindings
  % aufrufen
(
  (nonvar(Value))
...
rdfs_instance_of(Resource, Class) :-
  nonvar(Resource)
  -> ( nonvar(Class)
    -> ((rdfs_individual_of(Resource, Class) ; rdfs_computable_instance_of_subclass(Resource, Class)),!)
