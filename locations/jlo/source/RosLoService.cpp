...
void RosLoService::tf_subscription_callback(const tf::tfMessage::ConstPtr &msg_in_)
{
 ...
 /*   printf("got matrix:= \n[");
    ...
    else
    {
      ...
      {
          printf("tf from %s -> %s does not correspond with the actual situation %ld -> %ld\n", trans.frame_id_.c_str(), trans.child_frame_id_.c_str(), pose->m_uniqueID, pose->m_parentID);
          pose->m_mapstring = "";
          jlo::ServiceLocatedObject* parent = GetServiceLocatedObject(parentid);
          if(parent == NULL)
          {
