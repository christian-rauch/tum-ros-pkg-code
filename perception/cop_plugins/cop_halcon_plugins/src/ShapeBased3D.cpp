    ...
    else
    {
      ...
      (*m_paramList)[8] = numOfObjects;
        ...
        if(trackPossible)
        {
          ...
          GuidedPoseRestiction(m_paramList, m_paramNameList, model, 0.1, object.m_relPose, calib->CamParam(scale),&region, pm);
        ...
        if(Score.Num() > 0)
        {
          ...
          for(int i = 0; i < Score.Num(); i++)
          {
            ...
            {
              if(false && object.m_relPose != NULL && i == 0 && object.m_relPose->m_uniqueID!=ID_WORLD)
              {
                result.push_back(RelPoseHTuple::FRelPose(pose_sel, cov_sel, camPose, object.m_relPose->m_uniqueID));
