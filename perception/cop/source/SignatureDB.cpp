  ...
  s_shapeDownloader(NULL)
{
  ...
  s_shapeDownloader = new ShapeModelDownloader(this);
}

SignatureDB::~SignatureDB ( )
{
        delete s_shapeDownloader;
	delete m_dbStarter;
	delete m_index;
}
...
//
#ifdef BOOST_THREAD
void SignatureDB::AddSignatureThreaded(Signature* sig)
{
...
Signature* SignatureDB::GetSignatureByIndex(unsigned int index)
{
  if(index < m_dbStarter->CountChildren())
  {
    ...
    else
    {
     try
     {
       Signature* sig = new Signature(m_dbStarter->GetChild(index));
...
Elem* SignatureDB::FindCreateDescriptor(int class_id)
{
  ...
  else
  {
    ...
    printf("Trying to Download Descriptors for classes %s\n", searchstring.c_str());
    try
    {
      result = s_shapeDownloader->FindModels(searchstring);
    }
    catch(const char* text)
    {
      printf("SignatureDB: Downloading failed: %s\n", text);
    }
  }
  return result;
}
