...
#ifdef HALCONIMG
RelPose* RelPoseFactory::FRelPose(Halcon::HTuple& poseDesc, Halcon::HTuple& covariance, RelPose* relation, int id)
{
#ifdef NO_LO_SERVICE_AVAILABLE
  if(id != -1)
  {
    RelPose* pose = GetRelPose(id);
    if(pose != NULL)
      pose->Update(poseDesc, covariance, relation);
    return pose;
  }
  return GetRelPoseIndex(SetRelPose(new RelPose(poseDesc, covariance, relation)));
#else /*NO_LO_SERVICE_AVAILABLE*/
  if(id != -1 && relation != NULL)
  {
    RelPose* pose_own = GetRelPose(id);
    if(pose_own != NULL && pose_own->m_parentID == relation->m_uniqueID)
    {
      pose_own->Update(poseDesc, covariance, relation);
      s_loService->NotifyPoseUpdate(pose_own, true);
      return pose_own;
    }
    else
    {
      ...
      Matrix m(4,4);
      Matrix d(6,6);
      if(!RelPose::TupleToMat(poseDesc, covariance, m, d))
        printf("Error converting tuple to matrix in FRelPose\n");
      return s_loService->CreateNewPose(relation, &m, &d);
    }
...
#else /*NO_LO_SERVICE_AVAILABLE*/
    s_loService->FreePose(pose->m_uniqueID);
#endif  /*NO_LO_SERVICE_AVAILABLE*/
  }
