...
std::vector<RelPose*> FindCalTab::Perform(std::vector<Camera*> cam, RelPose* lastKnownPose, Signature& object, int &numOfObjects, double& qualityMeasure)
{
...
double FindCalTab::CheckSignature(Signature& object)
{
...
std::vector<RelPose*> FindCalTab::Inner(Image* img, RelPose* camPose,Calibration* calib,RelPose* lastKnownPose, CalTab* cm, int &numOfObjects, double& qualityMeasure)
{
  ...
  if(img->GetType() == HALCONIMAGE)
  {
    ...
    result.push_back(RelPoseFactory::FRelPose(FinalPose, cov,  camPose));
...
std::vector<RelPose*> FindCalTab::Inner(Image* img, RelPose* camPose,Calibration* calib,RelPose* lastKnownPose, Signature& object, int &numOfObjects, double& qualityMeasure)
{
  ...
  if(img->GetType() == HALCONIMAGE)
  {
    ...
    cov[4] = 0.4;
    cov[5] = 0.4;
    if(object.m_relPose != NULL && object.m_relPose->m_uniqueID!=ID_WORLD)
        {
            result.push_back(RelPoseFactory::FRelPose(finalpose, cov, camPose, object.m_relPose->m_uniqueID));
