...
LocateAlgorithm* cop::LocAlgFactory(XMLTag* tag)
{
  std::string name = tag->GetName();
	if(name.compare(XML_NODE_SHAPEBASED3DALG) == 0)
		return new ShapeBased3D(tag);
	else if(name.compare(XML_NODE_FINDCALTAB) == 0)
    return new FindCalTab(tag);
  else if(name.compare(XML_NODE_BLOBLOCALIZER) == 0)
    return new BlobLocalizer(tag);
	else if(name.compare(XLM_NODE_COLORBASED) == 0)
		return new ColorBased();
	else if(name.compare(XLM_NODE_COLORBASED) == 0)
		return new ColorBased();
	else if(name.compare(XML_NODE_SUPPORTINGPLANEDETECTOR) == 0)
		return new SupportingPlaneDetector();
	else if(name.compare(XML_NODE_SIMLOCATE) == 0)
		return new SimulatedLocate();
	else if(name.compare(XML_NODE_CHECKCOLORCLASS) == 0)
		return new CheckColorClass(tag);
	else if(name.compare(XML_NODE_TWOINONEALG) == 0)
		return new TwoInOneAlg(tag);
#ifdef SWISS_RANGER_SERVICE
	else if(name.compare(XML_NODE_CLUSTERDETECTOR) == 0)
		return new ClusterDetector(tag);
#endif
#ifdef DESCRIPTOR_AVAILABLE
	else if(name.compare(XML_NODE_DESCRIPTORBASEDALG) == 0)
		return new DescriptorBased();
	else if(name.compare(XML_NODE_COMBINEDSHAPEDESCRALG) == 0)
		return new CombinedShapeDescr(tag);
#endif
#ifdef DEFORMSHAPE_AVAILABLE
	else if(name.compare(XML_NODE_DEFORMSHAPEBASEDALG) == 0)
		return new DeformShapeBased(tag);
#endif
	else
	{
    pluginlib::ClassLoader<LocateAlgorithm> alg_loader("cop", "LocateAlgorithm");

    try
    {
      LocateAlgorithm* alg = alg_loader.createClassInstance(name);
      return alg;
    }
