...
/////////////////////////////////////////////////////////////////////////////////////
ODUFinder::ODUFinder() :
	camera_image(NULL), template_image(NULL), image(NULL), tree_builder(Feature::Zero())
  {
...
std::string ODUFinder::process_image(IplImage* camera_image_in) 
{
  ...
  else 
  {
    ...
    // @TODO: make this a parameter
		...
		std::string name;
		DocumentInfo** documents_to_visualize = new DocumentInfo*[IMAGES_TO_SHOW];
...
//		}
    ...
    //image names
		...
		int added = 0;
		for (int i=0; added<IMAGES_TO_SHOW && i < votes_count; ++i)
		{
			...
			if (position == std::string::npos)
      {
        position = d->name.find('.');
			...
			if (unique_names.find(short_name) == unique_names.end())
			{
				...
				logger << short_name.c_str() << "\t" << votes[i].second << std::endl;
                          ...
                          std::vector<KeypointExt*>& camera_keypoints) 
{
	...
	for (int i = 0; i < IMAGES_TO_SHOW; ++i) 
  {
		if (template_document_info[i] != NULL)
		{
			...
			template_image_file.append(template_document_info[i]->name.c_str());
			template_images[i] = cvLoadImage(template_image_file.c_str(), CV_LOAD_IMAGE_GRAYSCALE);
		}
		else
...
void ODUFinder::write_stat_summary()
{
	...
	for (int i=0; iter != stat_summary_map.end(); ++iter, ++i) 
  {
	...
	for (unsigned int i=0; i<pairs.size(); ++i)
		logger << pairs[i].first.c_str() << "\t" << pairs[i].second << std::endl;
	logger.close();
}
