...
int main (int argc, char  **argv)
{
 ...
 while (1)
   {
     ...
     static struct option long_options[] = {
       ...
       {"angle_filename", required_argument, 0, 'a'},
	 ...
	 break;
       case 'a':
	 g_options.angle_filename = atof(optarg);
	 break;
       case 'i':
 ...
 // Set the current actuator as the gripper (default)
  ...
  //homing all joint, does it still work?
  //homeall (client, device_actarray);
  //home 1 joint, does it still work?
  ...
  while (fabs(next_pos  -   RAD2DEG (device_actarray->actuators_data[g_options.rot_joint].position)) > EPS)
    {
      ...
      printf ("Angle  %f\n", fabs(next_pos  -   RAD2DEG (device_actarray->actuators_data[g_options.rot_joint].position)));
    }
